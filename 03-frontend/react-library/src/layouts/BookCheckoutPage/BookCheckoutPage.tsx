...
export const BookCheckoutPage = () => {
  ...
  const [isLoadingCurrentLoansCount, setIsLoadingCurrentLoansount] =
    useState(true);

  //Is Book Checked Out?
  const [isCheckedOut, setIsCheckedOut] = useState(false);
  const [isLoadingProductCheckedOut, setIsLoadingProductCheckedOut] =
    useState(true);
  ...
  useEffect(() => {
    const fetchUserCurrentLoansCount = async () => {
      ...
      setIsLoadingCurrentLoansount(false);
    };
  ...
  useEffect(() => {
    const fetchUserCheckedOutProduct = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        const CheckedOutProductResponse = await fetch(url, requestOptions);
        if (!CheckedOutProductResponse.ok) {
          throw new Error("Something went wrong");
        }
        const CheckedOutProductResponseJson =
          await CheckedOutProductResponse.json();
        setIsCheckedOut(CheckedOutProductResponseJson);
      }
